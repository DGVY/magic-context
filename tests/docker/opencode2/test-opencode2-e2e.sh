...
[[ "$(opencode2 --version)" == "opencode v2.0.3" ]] || {
    echo "FAIL unexpected OpenCode version: $(opencode2 --version)"
    exit 1
}
[[ "$(node -p "require('/test/host/node_modules/@opencode/cli/package.json').version")" == "2.0.3" ]]
[[ "$(node -p "require('/test/host/node_modules/@opencode/cli-linux-x64/package.json').version")" == "2.0.3" ]]
...
echo "PASS @opencode/cli@2.0.3 and @opencode/cli-linux-x64@2.0.3 exact pins"
echo "PASS GA TUI executed setup and painted the RPC-backed sidebar"
echo "All OpenCode 2.0.3 Docker E2E checks passed."
