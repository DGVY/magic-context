...
Each step pins two encodings of one result: `result_bytes_base64` is the bare typed result (`ScopeResult`, `TicketResult`, `PrepareResult`, `ResolveResult`, `BeginResult`, `PutResult`, `FinishResult`, `RedeemResult`), and `response_bytes_base64` is the clause-3a keyed response that wraps it. Operations are spelled as clause 3 names them (`scope.open`, `attempt.ticket`, `prepare`, `attempt.resolve`, `lineage.begin`, `capacity.begin`/`put`/`finish`/`check`), which is what keeps `capacity.put` and `lineage.put` distinct under R43c. An absent Option inside a typed DTO stays explicit `null` per R19, while `lineage_adopted_from` is the operation-style option R47a pins as omitted when absent.
