...
Each step pins two encodings of one result: `result_bytes_base64` is the bare typed result (`ScopeResult`, `TicketResult`, `PrepareResult`, `ResolveResult`, `BeginResult`, `PutResult`, `FinishResult`, `RedeemResult`), and `response_bytes_base64` is the clause-3a keyed response that wraps it. A step's own `op` field and that keyed response spell the operation as clause 3 names it (`scope.open`, `attempt.ticket`, `prepare`, `attempt.resolve`, `lineage.begin`, `capacity.begin`/`put`/`finish`/`check`). The `op` discriminator inside `request_bytes_base64` follows the clause 2 wire grammar instead, where those dotted clause 3 names are operation labels rather than wire values: the nine original operations are tagged `ticket`, `prepare`, `resolve`, `redeem`, `release`, `cancel`, `begin`, `put`, and `finish`, while `capacity.begin`/`put`/`finish`/`check` and `scope.open` keep the dotted discriminators they were pinned with (R48). That split is what keeps the scope-keyed `capacity.put` and the attempt-keyed source-segment `put` distinct under R43c, and the reference model accepts each operation under one spelling only. An absent Option inside a typed DTO stays explicit `null` per R19, while `lineage_adopted_from` is the operation-style option R47a pins as omitted when absent.
