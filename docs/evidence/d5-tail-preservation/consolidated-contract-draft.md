...
- What exact serialized tombstone-reporting shape should represent a generation-fenced logical REFUSED when the negative-row cap prevents allocating a receipt row, including immutable reconstruction of the terminal replay answer? The safety outcome, pre-dispatch ticket, no ticket refresh, explicit existing SEALED and REDEEMED results, and current p_fence response are fixed; only the reporting encoding remains to settle.
- When the successor's first actual outgoing geometry check fails despite a valid retained-envelope estimate, should the existing normal-transform path return a typed refusal immediately or invoke an already-authorized degradation lane? Neither option may silently discard carry, send overflow, resurrect a body, undo REDEEMED or remint the successor.
- Who produces and reviews the measured placeholder-plus-wrapper anchor_reserve constant and its measurement version for each serving encoding? MC owns the constant, receipt provenance and unknown-refusal behavior; the operational measurement owner is not assigned by the consolidated design.
