...
20. **Every-pass carry and digest validation.** TransformResponse.d5_carry serves carry after m0 and m1, before continuation, on first HARD, every DEFER, growth and restart for a redeemed D5 successor. serde defaults and omits none; absence means no receipt-backed carry this pass. Validated folds or reductions alone retire intervals; partial fold causes SOFT. Carry precedes retention and pruning. Member and digest validation is clause 21a; tags, versions, hosts and fresh digests authorize nothing.
...
21a. **Gateway carry-proof path.** While a receipt-backed carry is locally outstanding (REDEEMED, not discharged by verified fold, reduction or custody transfer), the gateway evaluates d5_carry first every pass, native anchor or not. Required: schema_version=1; the locally REDEEMED receipt_id for (P,agent,incarnation); its sealed manifest_digest; receipt-backed coverage_proof or real-compartment proof covering the required members at or beyond folded_frontier; row_version equal or higher than last accepted. Content proves custody: ordered members equal the returned ck_messages before gateway markers, each once in order; frozen served_sha256 under mc.d5.block.served.v1; reduced digest under mc.d5.unit-projection.v1; aggregate projection_digest under mc.d5.projection.v1; clause 2a forms only. Metadata, high ordinal, flag or empty boundary is insufficient. Missing: 503 d5_carry_proof_missing; mismatch: 503 d5_carry_proof_mismatch; never raw or upstream. Absence declares no served carry and discharges nothing. Native checks then run unchanged: envelope, provider shape, and the native-owned lineage_anchor, continuation_identity and rebase_base; after a valid proof the inherited anchor (coverage_identity, folded_frontier, first_inherited_ordinal) is never re-required natively; that was the original gap. Omitted carry fails beside a valid native anchor. Without outstanding carry only the native path runs with the pre-D5 anchor check; absence is normal. I54–I62 pin these.
...
27. **Fixtures, store fence and disjoint slices.** Slice 0 imports immutable owner data: d5-canonical-v1.json; I12 source, expected manifest and archive; I34's 141-message union; both named anchor JSON files; and policy-reserve-v3 inputs with the three R14 samples. OpenCode and Pi goldens are re-cited only. fixture-index-v1.json is the import manifest, listing each imported file's size, redaction, provenance and SHA-256; there is no directory census or identity-table JSON. Slice 0 produces nothing. Source digests are VACUUM f589668287f41abaeb2a6526ee6d6f9d162e7ed80b1650f1ca5ec0a45984b8c0 and sanitized calibration 79cc7ac955ed220fcc9b371597fd414e7c47280c4f2038fa47dad9844746a2ad. v3-derived-path512.txt is 1530 B, SHA-256 632fa5b75a1e691bbbc9556ef6ab0ae87d5681339261ac03d484ac4366f000c3, pre-R14 443 then 510; R14 max is 561. Missing input fails. Slice 0 closes through its three named controls and manifest hashes; I1–I62 cover product and gateway work.
   ...
   Rollback backport is out of scope. The substitute deployment guard blocks an older ck-mc lacking the D5 gate before it opens, transforms or admits a D5-bearing store, preserving bytes; no slice builds or executes that binary. Ungated downgrade is unsupported; the current route can return d5_downgrade_refused, and non-D5 stays unchanged.

   Slice 1 owns mc-store schema, CE1, upload, lifecycle, reads, CAS and deletion, then one fence-bearing coordinated ck-mc bounce by the release owner. Slice 2 owns lineage route, wire, budget and status attachment; dispatch must re-cite the ManagementSurface file and handle_session_status_value region in mc-module/src/lib.rs. Slice 3 owns carry, d5_carry, digests, identities, absorption semantics, overlap, rebind and nesting; dispatch must re-cite apply_once as the second coverage symbol beside coverage_ordinal_from_compartments. Slice 4 owns historian, ctx_expand, markers, parity and specimens. Hop cap 5 stays. Regions are disjoint; transform.rs is sequential. Release waits for gateway shared-contract asks; slice workers never restart.
...
33. **Retained normal envelope lifecycle.** Retain one atomic whole envelope per session-agent-lineage-incarnation: latest managed normal committed send, including forwarded retry and excluding summary, rejected and unmanaged sends. MC stores receipt evidence, not this mutable cache; unusable evidence is unknown.
   ...
   Constraint one — order: committed send, not completion, wins; ingress and durable causality order it. Identity must match; late completion cannot overwrite and newer unusable tombstones old.

   Constraint two — persistence: write pending{sequence}, envelope, clear, then send. A leftover marker is newer_unusable; failed persistence prevents send; a restart without persistence is budget_unknown. BootId alone does not invalidate, and records never mix.

   Constraint three — identity: compare normal-successor geometry, profile, guidance, tool and system hashes. Model mismatch is budget_model_mismatch; another field is budget_evidence_mismatch. Age is causal distance, not TTL; uncertainty is budget_unknown.

   Constraint four — witness: MC incarnation detects older restore. Foreign or inconsistent values fail; equal-incarnation and joint rollback remain undetectable, so uncertainty refuses.

   Constraint five — refusal: unusable evidence is pre-seal budget_unknown without fallback. AUTO resumes ordinary history, manual errors and UNKNOWN resolves under the current fence. Clause 24 handles post-seal actual overflow.
